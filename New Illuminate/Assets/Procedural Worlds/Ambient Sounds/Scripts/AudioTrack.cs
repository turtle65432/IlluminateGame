...
namespace AmbientSounds {
    ...
    internal class AudioTrack {
        ...
        private int NextClipIdx {
            get {
                if (isSynced && m_sequence != null) {
                    if (_nextClipIdx < 0) { //reset value so we need to add to trackQueuePos
                        if (++trackQueuePos >= RawClipData.Length) { //we hit the end so we need to reset queue
                            if ((m_sequence.m_syncType & SyncType.REPEAT) > 0) {
                        ...
                        _nextClipIdx = trackQueuePos;
                    }
                    if (m_sequence.m_randomizeOrder)
                        return trackQueue[trackQueuePos];
                    else
                        return trackQueuePos;
                } else {
                    if (_nextClipIdx < 0) {
                        if (m_isOutputDirect) {
                            if (DirectClipData == null)
                                _nextClipIdx = 0;
                            else {

                                if (m_sequence.m_randomizeOrder) { //pick a new random track
                                    _nextClipIdx = Helpers.GetRandom(0, DirectClipData.Length - 1);
                                } else { //move on to next track
                                    _nextClipIdx = currentClipIdx;
                                    if (++_nextClipIdx >= DirectClipData.Length)
                                        _nextClipIdx = 0;
                                }
                            }
                        } else {
                            if (RawClipData == null)
                                ...
                                if (m_sequence.m_randomizeOrder) { //pick a new random track
                                    _nextClipIdx = Helpers.GetRandom(0, RawClipData.Length - 1);
                                } else { //move on to next track
                                    _nextClipIdx = currentClipIdx;
                                    if (++_nextClipIdx >= RawClipData.Length)
                                        _nextClipIdx = 0;
                                }
                            }
                        }
