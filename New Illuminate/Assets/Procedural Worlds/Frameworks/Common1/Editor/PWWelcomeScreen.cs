...
namespace PWCommon1
{
    public class PWWelcomeScreen : EditorWindow, IPWEditor
    {
        ...
        private void OnEnable()
        {
            ...
            if (m_prodImgASk == null)
            {
                m_prodImgASk = Resources.Load("AmbientSkies" + PWConst.VERSION_IN_FILENAMES) as Texture2D;
                m_maxProdImgWidth = m_prodImgASk.width > m_maxProdImgWidth ? m_prodImgASk.width : m_maxProdImgWidth;
            }

            if (m_prodImgASo == null)
            {
                m_prodImgASo = Resources.Load("AmbientSounds" + PWConst.VERSION_IN_FILENAMES) as Texture2D;
                m_maxProdImgWidth = m_prodImgASo.width > m_maxProdImgWidth ? m_prodImgASo.width : m_maxProdImgWidth;
            }
            ...
            GUILayout.BeginHorizontal();
            {
               GUILayout.FlexibleSpace();
                GUILayout.BeginVertical();
                {
                    ...
                    m_editorUtils.ClickableImgDescriptionBold("ASkDescription", GUILayout.Width(m_maxProdImgWidth), GUILayout.Height(descriptionHeight));
